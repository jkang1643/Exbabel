        ...
        case 'init':
          ...
          if (!speechStream) {
            try {
              ...
              // Partial tracker reset timeout (for grace period at segment boundaries)
              let partialTrackerResetTimeout = null;
              const PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS = 500; // Wait 500ms before resetting to allow new segment to stabilize
                ...
                (async () => {
                  try {
                    ...
                    try {
                      ...
                      // FIX: Add grace period before reset to prevent word loss at segment boundaries
                      // The first word(s) of a new segment may arrive in incomplete partials before Google stabilizes
                      if (!isForcedFinal) {
                        // Clear any existing reset timeout to prevent multiple resets
                        if (partialTrackerResetTimeout) {
                          clearTimeout(partialTrackerResetTimeout);
                          partialTrackerResetTimeout = null;
                        }

                        // Save snapshot of current partial state for logging
                        const lastPartialSnapshot = latestPartialText || longestPartialText || '';

                        // Schedule delayed reset with grace period
                        partialTrackerResetTimeout = setTimeout(() => {
                          partialTracker.reset();
                          syncPartialVariables();
                          console.log(`[HostMode] 🧹 Reset partial tracking after ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms grace period (last partial: "${lastPartialSnapshot.substring(0, 50)}${lastPartialSnapshot.length > 50 ? '...' : ''}")`);
                          partialTrackerResetTimeout = null;
                        }, PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS);

                        console.log(`[HostMode] ⏳ Scheduled partial tracker reset in ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms (grace period for new segment)`);
                      }
              ...
              speechStream.onResult(async (transcriptText, isPartial, meta = {}) => {
                ...
                if (isPartial) {
                  ...
                  // CRITICAL FIX: Recover short/ignored partials when a new segment starts
                  // If we ignored a previous partial because it was too short (e.g. "Hello"), and now a new segment starts ("World"),
                  // we must commit the "Hello" partial effectively flushing it before processing "World".
                  if (isNewSegmentStart) {
                    const trackerSnapshot = partialTracker.getSnapshot();
                    const leftoverPartial = trackerSnapshot.latestPartialText;

                    // Only recover if valid leftover exists and isn't part of new text
                    if (leftoverPartial &&
                      leftoverPartial.length > 0 &&
                      leftoverPartial !== transcriptText &&
                      !transcriptText.startsWith(leftoverPartial)) {

                      console.log(`[HostMode] ⚠️ Recovering ignored short partial before new segment: "${leftoverPartial}"`);
                      console.log(`[HostMode]    Current partial (new segment): "${transcriptText}"`);

                      // Commit the leftover immediately to save it
                      processFinalText(leftoverPartial);

                      // Reset tracker to prevent duplicating it or merging it into new segment
                      // Also clear the reset timeout since we just handled the reset manually (via processFinalText -> reset)
                      if (partialTrackerResetTimeout) {
                        clearTimeout(partialTrackerResetTimeout);
                        partialTrackerResetTimeout = null;
                      }
                  ...
                  if (false && finalizationEngine.hasPendingFinalization()) {
                    ...
                    if (!extendsFinal && !hasWordOverlap && !startsWithFinalWord && timeSinceFinal > 500 && !mightBeFalseFinal) {
                      ...
                      // CRITICAL FIX: Recover uncommitted tail words from partial tracker before committing
                      // pendingFinalization.text may be stale if recent partials came in but weren't stabilized yet
                      // This is common in rapid-fire short segments where the end of the previous segment is in the tracker but not yet in pendingFinal
                      ...
                      // Check if tracker has a better version (must start with pending final to be safe)
                      // Normalize for comparison
                      const pendingNorm = textToCommit.trim().toLowerCase().replace(/[.,!?;:…\s]+$/, '');
                      const bestNorm = bestTrackerText.trim().toLowerCase();

                      if (bestTrackerText.length > textToCommit.length &&
                        (bestNorm.startsWith(pendingNorm) || bestNorm.includes(pendingNorm))) {
                        console.log(`[HostMode] ⚠️ Recovering uncommitted tail from tracker: "${bestTrackerText.substring(textToCommit.length)}"`);
                        console.log(`[HostMode]    Pending: "${textToCommit}"`);
                        console.log(`[HostMode]    Tracker: "${bestTrackerText}"`);
                      ...
                      // textToCommit is already defined above
                ...
                if (isForcedFinal) {
                  ...
                  if (endsWithPunctuation) {
                    ...
                    // FIX: Use grace period to prevent word loss at segment boundaries
                    if (partialTrackerResetTimeout) {
                      clearTimeout(partialTrackerResetTimeout);
                      partialTrackerResetTimeout = null;
                    }
                    ...
                    partialTrackerResetTimeout = setTimeout(() => {
                      partialTracker.reset();
                      syncPartialVariables();
                      console.log(`[HostMode] 🧹 Reset partial tracking after ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms grace period (forced final, last partial: "${lastPartialSnapshot.substring(0, 50)}${lastPartialSnapshot.length > 50 ? '...' : ''}")`);
                      partialTrackerResetTimeout = null;
                    }, PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS);
                    console.log(`[HostMode] ⏳ Scheduled partial tracker reset in ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms (forced final grace period)`);
                  } else {
                    ...
                    forcedCommitEngine.setForcedFinalBufferTimeout(() => {
                      ...
                      partialTrackerResetTimeout = setTimeout(() => {
                        partialTracker.reset();
                        syncPartialVariables();
                        console.log(`[HostMode] 🧹 Reset partial tracking after ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms grace period (forced final timeout, last partial: "${lastPartialSnapshot.substring(0, 50)}${lastPartialSnapshot.length > 50 ? '...' : ''}")`);
                        partialTrackerResetTimeout = null;
                      }, PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS);
                      console.log(`[HostMode] ⏳ Scheduled partial tracker reset in ${PARTIAL_TRACKER_RESET_GRACE_PERIOD_MS}ms (forced final timeout grace period)`);
