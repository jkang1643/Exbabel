...
function validateCorrectionResponse(corrected, original) {
  ...
  if (isErrorResponse) {
    console.warn(`[OpenAIGrammarProvider] ⚠️ AI returned error/question instead of correction: "${corrected.substring(0, 100)}..."`);
    console.warn(`[OpenAIGrammarProvider] ⚠️ Using original text instead: "${original.substring(0, 100)}..."`);
    return original;
  }
...
export class OpenAIGrammarProvider extends BaseGrammarProvider {
  constructor(config = {}) {
    super(config);
    this.name = 'openai';
  ...
  async correctPartial(text, options = {}) {
    // Logic extracted from grammarWorker._processPartialCorrection
    const apiKey = options.apiKey;
    const signal = options.signal;

    if (!apiKey) {
      console.error('[OpenAIGrammarProvider] ERROR: No API key provided');
      return text;
    }

    // Skip API call if rate limited
    if (isCurrentlyRateLimited()) {
      console.log(`[OpenAIGrammarProvider] ⏸️ Rate limited - skipping correction, returning original text`);
      return text;
    }

    try {
      console.log(`[OpenAIGrammarProvider] 🔄 Correcting PARTIAL (${text.length} chars) with ${this.model}: "${text}"`);

      const isReasoningModel = this.model.startsWith('o1') || this.model.startsWith('o3') || this.model.startsWith('gpt-5');
      ...
      console.log(`[OpenAIGrammarProvider] 🏁 Request complete in ${Date.now() - startTime}ms. Status: ${response.status}`);
      ...
      try {
        ...
        console.warn(`[OpenAIGrammarProvider] ⚠️ Failed to parse JSON response, using original text:`, parseError.message);
        corrected = text;
      }
      ...
      console.error(`[OpenAIGrammarProvider] ❌ Error (${text.length} chars): `, error.message);
      return text;
    }
  }

  async correctFinal(text, options = {}) {
    // Logic extracted from grammarWorker.correctFinal
    const apiKey = options.apiKey;
    const signal = options.signal; // Can be passed from upstream or created here if not provided, but usually worker handles timeout
    ...
    if (isCurrentlyRateLimited()) {
      console.log(`[OpenAIGrammarProvider] ⏸️ Rate limited - skipping FINAL correction, returning original text`);
      return text;
    }

    try {
      console.log(`[OpenAIGrammarProvider] 🔄 Correcting FINAL (${text.length} chars) with ${this.model}: "${text}"`);
      ...
      const response = await fetchWithRateLimit('https://api.openai.com/v1/chat/completions', {
        signal: signal, // Pass the signal if provided
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${apiKey}`
      ...
      console.error(`[OpenAIGrammarProvider] ❌ Final correction error (${text.length} chars): `, error.message);
      return text;
    }
  }
}
