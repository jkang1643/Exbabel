-- Migration: Add get_listening_quota_status RPC function
-- Purpose: O(1) quota remaining lookup including active listening spans
-- 
-- Returns:
-- - included_seconds_per_month: from plan
-- - used_seconds_mtd: from usage_monthly
-- - active_seconds_now: from listening_spans (in-flight)
-- - remaining_seconds: included - used - active

CREATE OR REPLACE FUNCTION public.get_listening_quota_status(p_church_id UUID)
RETURNS TABLE (
...
mtd AS (
  ...
  WHERE um.church_id = p_church_id
    ...
    AND um.metric = 'listening_seconds'
),
active AS (
  -- Calculate effective seconds for active spans
...
ALTER FUNCTION public.get_listening_quota_status(UUID) SECURITY DEFINER;
REVOKE ALL ON FUNCTION public.get_listening_quota_status(UUID) FROM anon;
REVOKE ALL ON FUNCTION public.get_listening_quota_status(UUID) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_listening_quota_status(UUID) TO service_role;

COMMENT ON FUNCTION public.get_listening_quota_status IS 
    'Returns instant quota status including active listening spans. O(1) reads from usage_monthly.';
