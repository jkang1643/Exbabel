...
BEGIN
    ...
    FOR credit_record IN 
        ...
        WHERE mode IS NULL
    LOOP
        -- Update existing row to be 'solo' mode with FULL original seconds
        UPDATE purchased_credits
        SET 
            mode = 'solo'
        WHERE id = credit_record.id;
        
        -- Create duplicate row for 'host' mode with FULL original seconds
